...
4. **Analysis scope** — all RAG/vector components: vector DB clients, embedding wrappers, prompt construction, hybrid builders, chunking utilities, and ingestion pipelines; exclude only `node_modules`, `dist`, `coverage`, `build`, `.next`, `.git`, `vendor`.
...
1. Execute deep scanning across all AI and vector integration components (`*.ts`, vector database clients, embedding generator wrappers, prompt construction modules, spatial-vector hybrid builders, and chunking strategies).
...
- **Backpressure & Batch Embeddings**: Sequential LLM embedding calls causing rate-limit spikes instead of optimized batch requests.
...
### TypeScript Strictness & Vector Typing
- Compile under `strict: true`; **`any` is forbidden** — vector payloads, embedding results, and LLM responses MUST be declared as exported `interface`/`type` with explicit field types.
...
- Deterministic vector IDs (`sha256(documentId:chunkIndex)`) for idempotent re-ingestion — flag duplicate inserts on re-index.
...
export class VectorSearchService {
  ...
  async search(query: VectorSearchQuery): Promise<ScoredPoint[]> {
    const { tenantId, embedding, limit } = query;
    return this.client.search('rag_chunks', {
      vector: embedding,
      limit: Math.min(limit, 50),
      filter: {
        must: [{ key: 'tenantId', match: { value: tenantId } }],
      },
      with_payload: ['documentId', 'chunkIndex'],
    });
  }
}
```
...
- **Affected Location**:
  - File: `src/modules/rag/services/vector-search.service.ts`
