...
Your task is to conduct an exhaustive Resilience, Multi-Tenancy, Data Privacy, and Disaster Recovery Audit of this Mezzio project. You will analyze circuit breakers, rate limiting, graceful shutdown, tenant data isolation (request context / schema / RLS), immutable audit logging, PII/GDPR data masking, and fault recovery readiness.

Your analysis must be exhaustive and systematic. Do not stop after finding the first gap—scan every module, tenant middleware, rate limiter middleware, audit log service, database connection strategy, and external integration client.

---

## Context

You are operating inside a production-grade **PHP 8.x + Mezzio/Laminas** multi-tenant repository compiled under strict static analysis. The scope spans tenant middleware, request-context propagation (PSR-7 attributes), rate-limiter middleware, audit-log services, DB connection strategies, external HTTP clients, and Redis caching. Every claim MUST be grounded in the actual repository state and evaluated against the mandatory Mezzio & PHP Engineering Standards below. This skill executes on **DeepSeek-V4 Flash**: instructions are stepwise, deterministic, and schema-driven so output stays parseable with minimal token overhead.
...
4. **Analysis scope** — all `*.php` sources incl. middleware, handlers, DB providers, HTTP clients, throttling configs, and audit loggers; exclude only `vendor`, `node_modules`, `dist`, `coverage`, `build`, `.next`, `.git`.
...
1. Execute deep scanning across all application source files (`*.php`, middleware, handlers, database providers, external HTTP clients, rate limiting configs, and audit loggers).
2. Evaluate circuit breaker patterns, tenant context leakage across async boundaries (queue consumers, cron), PII masking in logs/storage, and graceful termination handling.
...
- **Circuit Breakers & Retries**: Unprotected downstream HTTP calls (Guzzle without timeouts/retries, no `resiliencephp/resilience` or custom circuit breaker), missing backoff algorithms.
- **Throttling & Rate Limiting**: Missing or bypassable rate-limiting middleware (`mezzio/mezzio-throttling` or Redis-backed custom middleware) on sensitive or CPU-heavy endpoints.
- **Graceful Shutdown & Signal Handling**: Missing `SIGTERM`/`SIGINT` handling, unreleased active DB connections or queue jobs during shutdown (PHP-FPM graceful reload, Swoole/Octane workers, consumers).

### 2. Multi-Tenancy Data Isolation
- **Tenant Context Propagation**: Flawed tenant context handling resulting in cross-tenant context bleeding in queue consumers, cron jobs, or fibers; tenant context not attached to queued jobs.
...
- **PII & Data Masking**: Exposure of Personally Identifiable Information (PII) or confidential business fields in application logs (Monolog), error reports (Sentry), or database dumps.
...
- **Database Connection Pool Resilience**: Unhandled connection drops, missing auto-reconnect configurations, or pool starvation under high concurrent load (PDO persistent connections, Doctrine reconnect).
...
### PHP 8.x Strictness & Tenant Typing
- Every source file starts with `declare(strict_types=1);`; **`mixed` is forbidden** — the tenant context MUST be a strongly typed readonly DTO (`TenantContext`) carried on PSR-7 request attributes; flag array-typed request context.
...
- `TenantContext` resolved in middleware and attached to the request; propagated EXPLICITLY to queue jobs and cron commands (payload DTO fields) — flag context bleeding across async boundaries (consumers, timers, parallel workers).
...
- External calls need timeouts, retries with exponential backoff, and circuit breakers with typed fallbacks — flag unprotected downstream HTTP calls.
- Throttling: rate-limiting middleware with per-route budgets on sensitive/CPU-heavy endpoints; flag missing, bypassable, or globally-uniform limits.
- Graceful shutdown: `SIGTERM`/`SIGINT` handling plus explicit draining of DB pools and queues; flag missing handling and connection leaks.
...
final class TenantContextMiddleware implements MiddlewareInterface
{
    ...
    {
        $user = $request->getAttribute('requestUser');
        if (!$user instanceof RequestUser) {
            return new JsonResponse(['error' => 'Missing authentication context'], 401);
        }

        $tenant = new TenantContext(userId: $user->sub, tenantId: $user->tenantId, roles: $user->roles);

        return $handler->handle($request->withAttribute('tenantContext', $tenant));
    }
}

final class ScopedTaskRepository
{
    ...
    {
        $tenant = $request->getAttribute('tenantContext');
        if (!$tenant instanceof TenantContext) {
            throw new \RuntimeException('Missing tenant context'); // -> 403 via error middleware
        }
...
- **Affected Location**:
  - File: `src/App/Middleware/TenantContextMiddleware.php`
  - Class / Method: `TenantContextMiddleware::process`
  - Line Number(s): `22-58`
...
- **Modules / Middleware Analyzed**: Total Count
- **Impact Breakdown**:
  - Critical Impact: X
  - Major Impact: X
  - Moderate Impact: X
...
## Constraints
1. **Never Stop Early**: Scan every tenant middleware, HTTP service client, rate-limit middleware, database connection provider, and logger module.
